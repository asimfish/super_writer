...
## Evidence Gate Escalation (Optional)

The `Evidence required / available / missing` rows are self-reported by the
author. When the paper carries experimental claims and the project has real
results (experiment logs, W&B runs, result tables), two companion skills turn
that self-report into a checked verdict:

- **`result-to-claim`** — run it on the available results *before* locking the
  Core contribution statement. Its verdict maps directly onto this artifact:
  `claim_supported=yes` → keep the statement at `Strong claims allowed`
  strength; `partial` → move the unsupported part into `Claims to soften or
  avoid`; `no` → the Core statement must be rewritten, not hedged.
- **`ablation-planner`** — when the verdict is `yes`/`partial`, use it to
  design the ablations a reviewer will demand. Its output is the concrete
  content of `Evidence required`; anything planned but not yet run belongs in
  `Evidence missing`, not silently dropped.

Record the verdict inline in `confirmed_contribution.md` (e.g.
`result-to-claim: partial, 2026-07-03, see docs/claim_verdict.md`). If the
skills or the results are unavailable (pure-theory paper, materials-only
build), skip the escalation and leave the self-reported rows — do not fabricate
a verdict. `contribution_check.py` behavior is unchanged.
...
2. When experimental results exist, run the `result-to-claim` /
   `ablation-planner` escalation above before locking the Core statement.
3. Resolve `<SKILL_DIR>` to the directory containing the loaded `SKILL.md`. From the project directory, run `python "<SKILL_DIR>/scripts/contribution_check.py" paper_rewriting_output` until it passes (exit 0).
4. Only then proceed to motivation thread, section blueprints, and writing — each tracing back to the Core contribution.
5. Re-run the check whenever the claim or evidence changes; a shifted contribution invalidates downstream sections.
