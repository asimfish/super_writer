...
When the audit stage runs the optional `ensemble-reviewer` escalation
(`references/audit.md`), fold its findings the same way: R1 Methodology →
technical-soundness rows, R2 Experiments → evidence-sufficiency rows, R3
Writing → clarity rows, R4 Domain → novelty/positioning rows, R5 Adversarial →
boundary/fairness rows, and the Area Chair meta-review → the Editorial Fit
Map. Keep one register row per finding and cite the source reviewer (e.g.
`ENS-R4`) in the `Where triggered` column so scores stay traceable. If the
escalation was skipped, note `SKIPPED (ensemble-reviewer unavailable)` once at
the top of the register — do not invent rows for reviews that never ran.
