...
## Ensemble Review Escalation (Optional)

The built-in `structured_review.py` dispatches three reviewer personas
(Methods / Contribution / Clarity). For `scene=conference` or `scene=journal`
runs that need venue-grade scoring, escalate to the companion
`ensemble-reviewer` skill (5 reviewer roles + Area Chair, AgentReview-style)
after the three-persona review completes:

1. Read the sibling skill `ensemble-reviewer/SKILL.md` and run it against
   `final_paper/main.tex`, setting VENUE to the config `target_name`.
2. Save its outputs under `paper_rewriting_output/reviews/`.
3. Fold every CRITICAL/MAJOR finding and the Area Chair meta-review into the
   `reviewer_audit.md` Objection Register (mapping rules in
   `references/reviewer-audit.md`), one row per finding, Status OPEN.
4. Re-run `reviewer_audit_check.py` after folding.

This escalation is additive: the completion hard gate below stays unchanged,
and a missing `ensemble-reviewer` install or unavailable review model must not
block completion — record `SKIPPED (ensemble-reviewer unavailable)` in
`reviewer_audit.md` instead. Never fabricate ensemble scores.
