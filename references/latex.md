...
the template's mechanism: `\title` plus `\maketitle`, or an appropriate venue
macro such as `\icmltitle`. Preserve the venue template. Word output must begin with
...
- Low-quality figure escalation: if an included figure is a blurry raster of
  plottable data, regenerate it with the companion `paper-figure` skill from
  the raw data; if the architecture/teaser figure is missing entirely, produce
  it with `paper-illustration` (both from the build-stage escalation in
  `references/build.md`). Update `figure_asset_map.md`; skip silently is not
  allowed — note the gap in `latex_report.md` when the skills are unavailable.
