...
## Figure Production Escalation (Optional)

`figure_asset_map.md` inventories what exists; it does not create anything.
When mapped assets leave gaps, two companion skills close them — record every
generated file back into `figure_asset_map.md` with its source skill:

- **`paper-figure`** — data-driven plots and LaTeX tables (training curves,
  comparison bars, ablation tables) regenerated from the materials' raw data
  (CSV/JSON/W&B) instead of pasting low-resolution screenshots.
- **`paper-illustration`** — the architecture/pipeline/teaser figure (the
  Figure 1 a reviewer sees first) when the materials contain none.

Both write into `final_paper/figures/`. Escalation is additive: if the skills
are unavailable, keep the user-provided assets and note the gap in
`figure_asset_map.md`. Never generate a figure that implies results the
evidence bank does not contain — figure content is bound by the same
no-fabrication rule as prose.
