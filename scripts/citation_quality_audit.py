...
def parse_citation_rows(path: Path) -> list[dict[str, str]]:
    ...
    reference_names = ("reference/bibtex", "reference", "citation", "bibtex")
    if len(index) != len(header) or sum(name in index for name in reference_names) != 1:
        return []
    ...
    for row in rows:
        if len(row) < 6 or len(row) != len(header):
            continue
        reference = cell(row, *reference_names).strip()
        if not reference:
            continue
        # Never extract network identifiers from manuscript-support columns.
        dois = extract_dois(" ".join((reference, cell(row, "doi"), cell(row, "source"))))
        result.append({
