...
def collect_included_labels(text: str, base_dir: Path, seen: set[Path] | None = None, depth: int = 0) -> set[str]:
    """Collect \\label keys defined inside \\input/\\include files.

    LaTeX resolves \\input paths relative to the main compilation directory,
    so recursion keeps ``base_dir`` fixed instead of following each file's own
    parent directory.
    """
    labels: set[str] = set()
    if depth > 5:
    ...
    for match in re.finditer(r"\\(?:input|include)\{([^{}]+)\}", text):
        ...
        candidate = Path(name)
        relatives = [candidate] if candidate.suffix else [Path(name + ".tex"), candidate]
        for relative in relatives:
            path = (base_dir / relative).resolve()
            if not path.is_file() or path in seen:
                continue
            seen.add(path)
            sub = strip_comments(read_text(path))
            labels.update(re.findall(r"\\label\{([^{}]+)\}", sub))
            labels.update(collect_included_labels(sub, base_dir, seen, depth + 1))
            break
    return labels


def check_labels_and_refs(text: str, included_labels: set[str]) -> list[Finding]:
    findings: list[Finding] = []
    labels: dict[str, int] = {}
    ...
    for match in ref_pattern.finditer(text):
        ...
        for key in keys:
            if key not in labels and key not in included_labels:
                findings.append(Finding("warning", "reference", f"Reference `{key}` has no matching label (checked \\input files too).", line_number(text, match.start())))
    return findings
...
def check_placeholders(text: str) -> list[Finding]:
    findings: list[Finding] = []
    # Word boundaries keep package names like `todonotes` from being flagged;
    # standalone TODO/FIXME marks and real \todo{} notes still surface.
    for pattern in (r"\[NEED DATA:[^\]]*\]", r"\[NEED CITATION:[^\]]*\]", r"\bTODO\b", r"\bFIXME\b"):
...
def check_title_and_maketitle(text: str) -> list[Finding]:
    findings: list[Finding] = []
    # Venue classes ship their own title macros: icml (\icmltitle), acl/naacl
    # (\title via template), IEEEtran (\title), neurips (\title). Accept any
    # \...title{...} command so venue templates are not flagged.
    if not re.search(r"\\[a-zA-Z]*title(?:running)?\s*[\[{]", text):
        findings.append(Finding("error", "title", "Missing \\title{...}. Paper title is required."))
    if not re.search(r"\\maketitle|\\icmltitle\{|\\twocolumn\[", text):
...
def run_checks(tex_path: Path, bib_path: Path | None) -> list[Finding]:
    ...
    included_labels = collect_included_labels(text, tex_path.parent)
    findings: list[Finding] = []
    ...
    findings.extend(check_labels_and_refs(text, included_labels))
