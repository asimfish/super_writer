...
def _col_index(header: list[str], *names: str) -> int | None:
    """Match one explicit column, never a substring of a support-field label."""
    normalized = []
    for value in header:
    ...
    if len(set(normalized)) != len(normalized):
        return None
    matches = [i for i, value in enumerate(normalized) if value in names]
...
def verify_citation(
    ...
    *,
    ...
    ref_col = _col_index(header, "reference/bibtex", "reference", "bibtex", "citation")
    ...
    if ref_col is None:
        result.ok = False
        result.findings.append(
            "Missing or ambiguous Reference/BibTeX, Reference, BibTeX, or Citation column. "
            "No metadata was sent to Crossref."
        )
        return result
    ...
    for row_index, row in enumerate(rows):
        ...
        if len(row) != len(header) or not row[ref_col].strip():
            ...
            entry.note = "Malformed row or empty reference field; no metadata sent to Crossref"
            result.ok = False
