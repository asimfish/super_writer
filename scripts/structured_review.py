...
def validate_independence(review_dir: Path) -> dict:
    ...
    # local patch: prefer the reviewer OUTPUT files written per dispatch.md.
    # The *_reviewer.md files are prompt templates generated by --dispatch from
    # one shared template (same persona scaffold + same manuscript sections),
    # so comparing them always reports false "not independent" findings.
    # Fall back to *_reviewer.md for upstream-style directories that place the
    # review texts directly under the role file names.
    output_roles = ["methods_review_output.md", "contribution_review_output.md", "clarity_review_output.md"]
    prompt_roles = ["methods_reviewer.md", "contribution_reviewer.md", "clarity_reviewer.md"]
    roles = output_roles if all((review_dir / r).exists() for r in output_roles) else prompt_roles
    texts: dict[str, str] = {}
    ...
    role_names = [r.replace("_review_output.md", "").replace("_reviewer.md", "") for r in roles]
    ...
    return {
        ...
        # local patch: no strict= kwarg so the script still runs on Python 3.9
