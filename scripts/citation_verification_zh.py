...
class CitationCheckZH:
    candidate_id: str
    reference_text: str
    status: str = "INCOMPLETE"  # VERIFIED / SUSPICIOUS / INCOMPLETE / FAKE
...
def check_citation_bank_zh(out_dir: Path) -> CitationVerificationZHResult:
    ...
    for value in header:
        ...
        normalized.append(re.sub(r"\s*/\s*", "/", value))
    index = {value: i for i, value in enumerate(normalized)}
    reference_columns = [index[name] for name in ("reference/bibtex", "reference", "citation", "bibtex") if name in index]
    id_columns = [index[name] for name in ("candidate id", "id") if name in index]
    if len(index) != len(header) or len(reference_columns) != 1 or len(id_columns) != 1:
        result.suspicious = 1
        result.checks.append(CitationCheckZH(
            "schema", "", "SUSPICIOUS", issues=[
                "Missing or ambiguous ID/reference columns; no metadata sent to Crossref"
            ],
        ))
        return result

    ref_col, id_col = reference_columns[0], id_columns[0]
    for row_number, row in enumerate(rows, 1):
        candidate_id = row[id_col].strip() if id_col < len(row) else f"row{row_number}"
        if len(row) != len(header) or not row[ref_col].strip():
            result.suspicious += 1
            result.checks.append(CitationCheckZH(
                candidate_id, "", "SUSPICIOUS", issues=[
                    "Malformed row or empty reference field; no metadata sent to Crossref"
                ],
            ))
            continue
        ...
        channel_col = index.get("source channel")
        if channel_col is not None and row[channel_col].strip().lower() == "local":
            check.issues.append("Local source; not checked against Crossref")
            result.incomplete += 1
